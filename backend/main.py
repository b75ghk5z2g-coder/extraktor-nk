...
from extractor import detect_reading_stage, find_matches_in_text, find_reading_stage
from parsers import parse_document, parse_document_preview_html
from scraper import (
    build_session,
    download_file,
    extract_cpt_agenda_items,
...
def _process_cpt_page(session, cpt_url: str, agenda_title: Optional[str] = None):
    """
    ...
    `agenda_title`: the agenda item's own title text, if already known (the
    caller crawling a program page has this from extract_cpt_agenda_items).
    Some agenda items -- e.g. "Správa o výsledkoch kontrolnej činnosti
    Najvyššieho kontrolného úradu SR" (tlač 1204) -- are reports FROM NKÚ
    itself and have no Dôvodová/Spoločná správa attachment at all, so there
    is nothing for pick_explanatory_document to find. Previously that meant
    the item was marked "failed" even when its own title plainly names NKÚ.
    Now, when no explanatory document is found, the title is checked as a
    fallback before giving up -- via `agenda_title` if the caller passed
    one, otherwise by fetching the ČPT page's own heading (extract_cpt_title).
    """
    ...
    if target is None:
        # No Dôvodová/Spoločná správa to analyze -- fall back to the agenda
        # item's own title before treating this as a dead end.
        title_text = agenda_title or extract_cpt_title(cpt_soup)
        if title_text:
            title_matches = find_matches_in_text(title_text)
            if title_matches:
                label = f"{title_text} (názov bodu programu, tlač: {cpt_url})"
                reading_stage = detect_reading_stage(title_text)
                if reading_stage:
                    label += f" — {reading_stage}"
                matches = [
                    MatchResult(
                        source=label,
                        source_url=cpt_url,
                ...
                return matches, cpt_url, None

        logger.info(
            "No Dôvodová/Spoločná správa found on %s, and its title "
            "(if any) has no NKÚ mention either",
            cpt_url,
        )
        return [], None, cpt_url
    ...
    # VERIFIED 2026-08-06 (real text supplied by user): the reading stage is
    # stated as a suffix on the agenda item's own title as printed on the
    # program page -- "...(tlač 1077) - druhé čítanie" -- not on the
    # explanatory document's first page as originally assumed. Check the
    # title first (when the caller has one -- see extract_cpt_agenda_items),
    # and only fall back to scanning the document itself if the title didn't
    # say (e.g. a single pasted ČPT URL with no agenda_title in hand).
    reading_stage = detect_reading_stage(agenda_title) if agenda_title else None
    if not reading_stage:
        reading_stage = find_reading_stage(blocks)
    if reading_stage:
        label += f" — {reading_stage}"
...
def _run_scrape(url: str, session, job: Optional["ScrapeJob"] = None) -> ScrapeResponse:
    ...
    if is_document_preview_url(url):
        ...
        if reading_stage:
            for match in matches:
                match.source = f"{match.source} — {reading_stage}"
        return ScrapeResponse(
            document_count=1,
    ...
    cpt_items = extract_cpt_agenda_items(soup, url)
    ...
    if cpt_items:
        ...
        report(total=len(cpt_items))
        for idx, item in enumerate(cpt_items, start=1):
            report(current=idx, label=item.url)
            matches, processed_url, failed_url = _process_cpt_page(
                session, item.url, agenda_title=item.label
            )
            ...
            # A full schôdza can have ~200 tlače, each pulling a multi-MB
            # document preview (see fetch_document_preview's docstring). On
            # a memory-constrained host (e.g. Render's free 512 MB plan),
            # letting that accumulate over ~200 iterations before Python's
            # normal garbage collection gets around to it risks an OOM
            # crash mid-run. Drop references and collect explicitly instead.
            del matches
            gc.collect()

        return ScrapeResponse(
            document_count=len(cpt_items),
    ...
    for idx, link in enumerate(links, start=1):
        ...
        # Same reasoning as the ČPT loop above: free per-item memory
        # promptly instead of letting it accumulate across a long crawl.
        del blocks, matches
        gc.collect()
