...
def download_file(session: requests.Session, url: str, timeout: int = 30) -> Optional[bytes]:
    """Download a single document with a polite delay. Returns raw bytes, or None on failure."""
    time.sleep(REQUEST_DELAY_SECONDS)
    try:
        resp = session.get(url, timeout=timeout)
    except requests.RequestException as exc:
...
def fetch_document_preview(session: requests.Session, url: str, timeout: int = 60) -> Optional[str]:
    """
    Fetch a nrsr.sk DocumentPreview.aspx page. Returns decoded HTML text, or
    None on failure. Uses a longer timeout than fetch_session_page/download_file
    because a single response can be several MB (a full multi-page document
    rendered as HTML in one request -- see DOCUMENT_PREVIEW_RE docstring).
    """
    time.sleep(REQUEST_DELAY_SECONDS)
    try:
        resp = session.get(url, timeout=timeout)
    ...
    if resp.status_code == 403:
        ...
        return None
    if not resp.ok:
        logger.error("Unexpected status %s fetching document preview %s", resp.status_code, url)
        return None
