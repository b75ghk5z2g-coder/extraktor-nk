...
# 512 MB of RAM. A single unusually large or complex attached PDF/DOCX can
# by itself use several times its own file size once loaded and parsed
# (pdfplumber in particular is memory-hungry on complex/scanned PDFs), which
# was enough to OOM-crash a scan that otherwise processes documents one at
# a time. Rather than try to predict how much memory a given file will need,
# just skip outlier-sized files entirely -- a 20 MB+ attachment is unusual
# for these legislative materials, and skipping it (logged, counted as
# "failed"/skipped) is far better than crashing the whole run.
...
def download_dokument(
    ...
    Streams the response instead of loading it in one go, and aborts (returns
    None, as if the download had failed) if the file turns out to be larger
    than max_bytes -- see MAX_DOCUMENT_BYTES above for why. This is checked
    both via the Content-Length header (fast path, when the server sends
    one) and by counting bytes as they arrive (in case it doesn't).
    """
    time.sleep(REQUEST_DELAY_SECONDS)
    ...
    try:
        with session.get(url, timeout=timeout, stream=True) as resp:
            if not resp.ok:
                logger.error(
                    "Unexpected status %s downloading document %s", resp.status_code, doc_uuid
                )
                return None
            ...
            if content_length is not None:
                try:
                    if int(content_length) > max_bytes:
                        logger.warning(
                            "Skipping document %s: reports %.1f MB, over the %.0f MB cap",
                            doc_uuid, int(content_length) / 1_000_000, max_bytes / 1_000_000,
                        )
                        return None
                except ValueError:
            ...
            for chunk in resp.iter_content(chunk_size=65536):
                ...
                if total > max_bytes:
                    logger.warning(
                        "Skipping document %s: exceeded the %.0f MB cap while downloading",
                        doc_uuid, max_bytes / 1_000_000,
                    )
                    return None
                chunks.append(chunk)
