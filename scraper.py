...
def extract_cpt_agenda_items(soup: BeautifulSoup, base_url: str) -> List[DiscoveredLink]:
    ...
    agenda item's title -- e.g. "Správa o výsledkoch kontrolnej činnosti
    Najvyššieho kontrolného úradu Slovenskej republiky za rok 2025 (tlač
    1204)".

    VERIFIED 2026-08-06 (real program page markup, screenshot supplied by
    user): the <a href="...sid=zakony/cpt..."> link's own text is just the
    bare tlač number (e.g. "1204") -- the actual descriptive title sits in
    a <b>/<strong> tag in a sibling <td> within the same <tr>, not inside
    the link itself. So the title is read from the row, preferring the
    bold element if present and falling back to the row's full text (and,
    as a last resort, the link's own text) if the markup varies.
    ...
    for a in soup.find_all("a", href=True):
        ...
        if row is not None:
            bold = row.find(["b", "strong"])
            if bold:
                label = bold.get_text(strip=True)
            if not label:
                label = row.get_text(separator=" ", strip=True)
        if not label:
